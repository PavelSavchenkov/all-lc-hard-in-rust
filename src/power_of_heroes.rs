//! Solution for https://leetcode.com/problems/power-of-heroes
...
impl Solution {
    pub fn sum_of_power(mut a: Vec<i32>) -> i32 {
        ...
        let mut pow2 = vec![Num::new(1); n];
        for i in 1..pow2.len() {
            pow2[i] = pow2[i - 1] * Num::new(2);
        }

        let mut ans = Num::new(0);
        let mut sum_i = Num::new(0);
        for j in 0..n {
            let coef = Num::new(a[j] as u32) * Num::new(a[j] as u32) * pow2[j];
            ans += coef * sum_i;
            sum_i += Num::new(a[j] as u32) / pow2[j];
        }
        ans /= Num::new(2);

        for i in 0..n {
            let x = Num::new(a[i] as u32);
            ans += x * x * x;
        }
...
mod tests {
    ...
    fn case(#[case] nums: Vec<i32>, #[case] expected: i32) {
        let actual = Solution::sum_of_power(nums);
