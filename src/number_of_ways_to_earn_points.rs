//! Solution for https://leetcode.com/problems/number-of-ways-to-earn-points
...
impl Solution {
    pub fn ways_to_reach_target(target: i32, types: Vec<Vec<i32>>) -> i32 {
        ...
        let mut dp = vec![Num::new(0); target + 1];
        dp[0] = Num::new(1);
        for t in &types {
...
mod tests {
    ...
    #[rstest]
    #[case(6, vec![vec![6,1],vec![3,2],vec![2,3]], 7)]
    #[case(5, vec![vec![50,1],vec![50,2],vec![50,5]], 4)]
    #[case(18, vec![vec![6,1],vec![3,2],vec![2,3]], 1)]
    fn case(#[case] target: i32, #[case] types: Vec<Vec<i32>>, #[case] expected: i32) {
        let actual = Solution::ways_to_reach_target(target, types);
