//! Solution for https://leetcode.com/problems/valid-permutations-for-di-sequence
...
impl Solution {
    pub fn num_perms_di_sequence(s: String) -> i32 {
        ...
        let mut dp = vec![vec![Num::new(0); n + 1]; n + 2];
        dp[1][0] = Num::new(1);
        for len in 1..n + 1 {
            for last in 0..len {
                let cur_dp = dp[len][last];
                for next in 0..=len {
                    let mut real_last = last;
                    if real_last >= next {
                        real_last += 1;
                    }
                    assert!(real_last != next);
                    if real_last > next && s[len - 1] == b'I' {
                        continue;
                    }
                    if real_last < next && s[len - 1] == b'D' {
                        continue;
                    }
                    dp[len + 1][next] += cur_dp;
                }
            }
        }
...
mod tests {
    ...
    fn case(#[case] s: String, #[case] expected: i32) {
        let actual = Solution::num_perms_di_sequence(s);
