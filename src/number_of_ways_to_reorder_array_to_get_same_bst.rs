...
fn solve(a: Vec<i32>, binom: &Vec<Vec<Num>>) -> Num {
    if a.is_empty() {
        return Num::new(1);
    }
    ...
    let coef = binom[left.len() + right.len()][left.len()];
    solve(left, binom) * solve(right, binom) * coef
}

impl Solution {
    pub fn num_of_ways(a: Vec<i32>) -> i32 {
        let n = a.len();
        ...
        let mut ans = solve(a.clone(), &binom);
        ans -= Num::new(1);
        ans.val as i32
    }
}
