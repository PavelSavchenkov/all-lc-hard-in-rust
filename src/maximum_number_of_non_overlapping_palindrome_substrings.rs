//! Solution for https://leetcode.com/problems/maximum-number-of-non-overlapping-palindrome-substrings
    ...
    pub fn max_palindromes(s: String, k: i32) -> i32 {
        ...
        for i in 0..=n {
            if i >= k {
                for j in (0..=i - k).rev() {
                    let len = i - j;
                    if len >= k && pal.is_palindrome(j, len) {
                        dp[i] = dp[j] + 1;
                        eprintln!("{} --> {}", i, j);
                        break;
                    }
...
mod tests {
    ...
    fn case(#[case] s: String, #[case] k: i32, #[case] expected: i32) {
        let actual = Solution::max_palindromes(s, k);
