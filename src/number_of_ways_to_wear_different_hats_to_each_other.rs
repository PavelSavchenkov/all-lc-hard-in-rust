//! Solution for https://leetcode.com/problems/number-of-ways-to-wear-different-hats-to-each-other
...
impl Solution {
    pub fn number_ways(hats: Vec<Vec<i32>>) -> i32 {
        ...
        for i in 0..n {
            for &hat in &hats[i] {
                masks[(hat - 1) as usize] |= 1 << i;
            }
        }

        let mut dp = vec![Num::new(0); 1 << n];
        dp[0] = Num::new(1);
        for j in 0..N {
            for mask in (0..1 << n).rev() {
                let cur_dp = dp[mask];
                for i in 0..n {
                    if !bit(mask, i) && bit(masks[j], i) {
                        dp[mask | (1 << i)] += cur_dp;
                    }
                }
            }
...
mod tests {
    ...
    #[rstest]
    #[case(vec![vec![3,4],vec![4,5],vec![5]], 1)]
    #[case(vec![vec![3,5,1],vec![3,5]], 4)]
    #[case(vec![vec![1,2,3,4],vec![1,2,3,4],vec![1,2,3,4],vec![1,2,3,4]], 24)]
    fn case(#[case] hats: Vec<Vec<i32>>, #[case] expected: i32) {
        let actual = Solution::number_ways(hats);
