//! Solution for https://leetcode.com/problems/find-the-count-of-monotonic-pairs-ii
...
impl Solution {
    pub fn count_of_pairs(nums: Vec<i32>) -> i32 {
        ...
        for i in 1..n {
            let mut ndp = vec![Num::new(0); M + 1];
            let mut pref = vec![Num::new(0); M + 2];
            for x in 0..dp.len() {
                pref[x + 1] = pref[x] + dp[x];
            }

            for xx in 0..=nums[i] {
                let bound = (nums[i - 1] as i32 - nums[i] as i32 + xx as i32)
                    .min(M as i32)
                    .min(xx as i32);
                if bound < 0 {
                    continue;
                }
                ndp[xx] = pref[bound as usize + 1];
            }
            dp = ndp;
        }
...
mod tests {
    ...
    #[rstest]
    #[case(vec![2,3,2], 4)]
    #[case(vec![5,5,5,5], 126)]
    fn case(#[case] nums: Vec<i32>, #[case] expected: i32) {
        let actual = Solution::count_of_pairs(nums);
