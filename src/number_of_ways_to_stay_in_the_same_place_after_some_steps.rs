//! Solution for https://leetcode.com/problems/number-of-ways-to-stay-in-the-same-place-after-some-steps
//! 1269. Number of Ways to Stay in the Same Place After Some Steps

impl Solution {
    pub fn num_ways(steps: i32, n: i32) -> i32 {
        let n = n.min(steps / 2 + 1) as usize;
        let steps = steps as usize;

        let mut dp = vec![Num::new(0); n];
        dp[0] = Num::new(1);
        for step in 0..steps {
            let mut ndp = vec![Num::new(0); n];
            for i in 0..n {
                ndp[i] += dp[i];
                if i > 0 {
                    ndp[i - 1] += dp[i];
...
mod tests {
    ...
    #[case(2, 4, 2)]
    #[case(4, 2, 8)]
    fn case(#[case] steps: i32, #[case] arr_len: i32, #[case] expected: i32) {
        let actual = Solution::num_ways(steps, arr_len);
